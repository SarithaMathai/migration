...
Every `Blocked by:` edge in the program, in one picture. Solid arrows are the 7 "shared infrastructure"
blocks (six wait on product's `WriteSaga` module; one — impression's `F-01` — waits on `PRODUCT-BE-B-01`,
a plain intra-subgraph story, not a saga dependency); dashed arrows are the 8 "not-yet-live subgraph"
