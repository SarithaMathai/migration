...
As of this writing: **7** stories are blocked on shared infrastructure in another *phase-1* domain (6
wait on `PRODUCT-BE-E-00`, the shared `WriteSaga` module; 1 — impression's own `F-01` — waits on the
plain intra-subgraph story `PRODUCT-BE-B-01`), and **8** are blocked on a subgraph that hasn't federated
yet (5 of those are product's own Phase H TechPack contributions waiting on
