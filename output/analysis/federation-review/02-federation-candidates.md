...
| `ResourcesCount.*` stub lists (`productAttachments`…`watchlists`) | 8 domains | `[ID]` | **keep by design** — TechPack count pattern; owning subgraphs fill via `@requires` (reference-federation-patterns §9) | KEEP |
