...
> Rule of thumb applied: a `@DgsEntityFetcher` (reference resolver) is required **only when a *separate* subgraph (or the gateway) can hand the owner a representation** — co-located references inside the `plm-product` monorepo resolve in-process and need none (per federation-patterns-condensed §0).
