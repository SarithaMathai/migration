# Phase 4: Migration Plan & Stories — Impression
...
- [ ] Unit: `searchByProductId_withFilters` — `partnerIds` and `workspaceIds` appear as repeated params in the REST request
- [ ] Unit: `searchByProductId_empty` — empty product id returns `[]`, no exception thrown
- [ ] Unit: `dateTimeSerde` — `DateTime` field round-trips ISO-8601 *(one-time gate)*
- [ ] Integration: `searchImpressionsByProductId` via DGS test client returns impressions with correct structure
...
- [ ] Unit: `getImpressionCounts_returnsListAsParent` — impressions list is returned as the `ImpressionCount` parent
- [ ] Integration: `{ getImpressionCountsByProductId { counts { bpType counts } } }` via DGS test client returns expected shape
...
- [ ] Unit: `updateImpressions_happy` — PUT body is snake_case; both `impressionsToDelete` and `impressionsToUpdate` are present
- [ ] Unit: `updateImpressions_validationError` — `validationErrors` in response → `ImpressionValidationException` thrown
- [ ] Integration: mutation via DGS test client updates impressions and returns the updated list
...
- [ ] Unit: `productImpressions_inProcess` — resolver calls `impressionService`; no HTTP call is made
- [ ] Integration: `Product { impressions { id } }` query resolves in-process without a gateway hop
- [ ] Parity: DGS response matches spark-internal-graphql for a representative product
...
- [ ] Unit: `businessPartners_mapping` — partner ids are correctly mapped to `loadBpsWithType` args
- [ ] Unit: `workspaces_emptyContext` — empty workspace context returns `[]` without making a service call
- [ ] Unit: `createdBy_nullId` — `null` creator id returns `null`, no exception
- [ ] Integration: `Impression { createdBy { id } businessPartners { bpId } }` resolves in federated context
...
- [ ] Unit: `counts_perPartner` — each partner id produces a row with the correct filtered count
- [ ] Unit: `counts_totalRow` — the `totalCount` row is always last and equals the full impressions length
- [ ] Unit: `counts_errorFallback` — exception during product fetch returns `[{totalCount:0}]`, not a thrown error
- [ ] Integration: `{ counts { bpType counts } }` via DGS test client returns partner rows and the total row
