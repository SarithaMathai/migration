...
> **Source of truth:** `schemas/SPARK_Bom.graphqls` (988-line SDL) + `resolvers/product/SPARK_Bom.js` + `services/product/Bom.js` + `utils/bomUtils.js`
...
| File | Lines | Role |
|---|---|---|
| `schemas/SPARK_Bom.graphqls` | 988 | the source SDL — 13 queries, 6 mutations, `Bom`/`Bom_Unified`/material+impression interfaces, inputs |
| `resolvers/product/SPARK_Bom.js` | 735 | All BOM resolvers: 13 queries, 6 mutations, 20 type blocks (incl. 2 `__resolveType`) + 3 module-level helpers |
| `services/product/Bom.js` | 184 | REST client — 17 methods against `bom/v1` + master-data |
| `utils/bomUtils.js` | 325 | Trim size/caption dispatchers (15 `TRIM_TYPES`), field-resolver helpers, material dispatcher |
...
```
resolvers/SPARK_Bom.js (resolver)
...
7. **Circular import** — `bomUtils` imports `MATERIAL_CATEGORY_ID` from the resolver (`utils/bomUtils.js:6`).
...
9. **Latent bug** — `getHubMaterial` (`utils/bomUtils.js:268-278`) calls `getUserPermissionsJWT` **without
