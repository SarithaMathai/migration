...
> **Source of truth:** `schemas/SPARK_Watchlist.graphqls` (98-line SDL) + `resolvers/product/SPARK_Watchlist.js` + `services/product/Watchlist.js`
...
(`services/product/Watchlist.js:10`):
...
> [federation-patterns-condensed.md §0](../../../fedMigrationScripts/reference/federation-patterns-condensed.md)). The product analysis's
...
| File | Lines | Role |
|---|---|---|
| `schemas/SPARK_Watchlist.graphqls` | 98 | the source SDL — 4 queries, 3 mutations, `Watchlist`/inspection/partner types, inputs |
| `resolvers/product/SPARK_Watchlist.js` | 129 | 4 queries, 3 mutations, 3 type blocks (13 field resolvers) |
...
services/WatchlistService.js uses postOne, putOne, loadListing, loadOne, convertFunctions
```
...
- `updateWatchlistEntries`, then (🔴 attachment) bulk-archive `removedAttachmentIds`.
- **Latent bug:** the per-entry `watchlistEntries.map(async …)` is **not awaited** (no `Promise.all`/`await`) before the watchlist update — the user-group upserts race with the update.
- Fix on port.
