...
- businessPartners, otherReason, notes, inspections, version, createdAt, updatedAt}`; `WatchlistInspection.{id, type, instructions}`; `WatchlistPartner.{partnerId, partnerType, systemTeamId}`; all of `WatchlistInspectionAction` — DTO-mapped, no resolver.
- Covered by A02 + B01.
