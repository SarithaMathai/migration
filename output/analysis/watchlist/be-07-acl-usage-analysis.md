# Phase 7: ACL Usage Analysis — Watchlist
...
|---|---|---|---|
| `Mutation.cloneFilesForWatchlist` | downstream-token | `getUserPermissionsJWT` token minted then passed into `ctx.loaders.attachment.*(permissionJWT)` — capability token required to call **separate DGS (plm-attachment)** (`attachment`) | Mid-Request ACL Update — SparkSecurityService.updateCurrentUserPermissions(capabilityToken) refreshes the thread's security context before the downstream call to `attachment`, avoiding re-authentication |
| `Mutation.updateWatchlistEntries` | downstream-token | `getUserPermissionsJWT` token minted then passed into `ctx.loaders.attachment.*(permissionJWT)` — capability token required to call **separate DGS (plm-attachment)** (`attachment`) | Mid-Request ACL Update — SparkSecurityService.updateCurrentUserPermissions(capabilityToken) refreshes the thread's security context before the downstream call to `attachment`, avoiding re-authentication |
| `Query.getWatchlistByFilter` | own-domain-token | `getUserPermissionsJWT` token passed to this domain's own `watchlist` loader — capability token required for the resolver's own write/read, not a cross-domain call | No action needed — token gates the resolver's own read/write, stays resolver-local |
| `Query.getWatchlistByIds` | own-domain-token | `getUserPermissionsJWT` token passed to this domain's own `watchlist` loader — capability token required for the resolver's own write/read, not a cross-domain call | No action needed — token gates the resolver's own read/write, stays resolver-local |
| `Query.parentId` | own-domain-token | `getUserPermissionsJWT` token passed to this domain's own `watchlist` loader — capability token required for the resolver's own write/read, not a cross-domain call | No action needed — token gates the resolver's own read/write, stays resolver-local |
...
|---|---|
| `Mutation.cloneFilesForWatchlist` | Mid-Request ACL Update — SparkSecurityService.updateCurrentUserPermissions(capabilityToken) refreshes the thread's security context before the downstream call to `attachment`, avoiding re-authentication |
| `Mutation.updateWatchlistEntries` | Mid-Request ACL Update — SparkSecurityService.updateCurrentUserPermissions(capabilityToken) refreshes the thread's security context before the downstream call to `attachment`, avoiding re-authentication |
...
---
**Phase Completed:** Phase 7 — ACL Usage Analysis · **Domain:** `watchlist` · **ACL call sites:** 5 · **Downstream-token:** 2
