...
| `SampleAsset` union correctness | 🟡 Medium | Type resolver + per-member tests |
...
| 2 | `SampleAsset` union members + prefix rules (beyond Color/Artwork?) | B01 | Product Owner |
| 3 | Delete or `@deprecated` the 3 drift mutations; drop/undrop ownership (sample vs workspace) | F02 | Product Owner |
