...
> **Source of truth:** `schemas/SPARK_Claims.graphqls` (157-line SDL) + `resolvers/product/SPARK_Claims.js` + `services/Claim.js`
...
- There is no `context.js` in the snapshot.
- Unlike the product-family services, `ClaimService extends SparkService` and takes the **base URL directly** (`this.endpoint = endpoint`, `services/Claim.js:13`) — there is
...
| File | Lines | Role |
|---|---|---|
| `schemas/SPARK_Claims.graphqls` | 157 | the source SDL — 7 queries, 6 mutations, `Claims`/details/substantiate/export types, inputs |
| `resolvers/product/SPARK_Claims.js` | 164 | 7 queries, 6 mutations, 4 type blocks (17 field resolvers) |
...
services/ClaimService.js extends SparkService; uses postOne, putOne, loadListing, loadOne, convertFunctions, qs
```
...
- `user-profile`, `VMM` are **cross-subgraph** (true federation / gateway stitch).
- It **contributes** the `Product.claims` field and the TechPack `ResourcesCount.claims` count back to the `plm-product` subgraph.
