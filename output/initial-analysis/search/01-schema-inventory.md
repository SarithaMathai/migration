...
> **Source of truth:** `schemas/SPARK_Search.graphqls` (655-line SDL) + `resolvers/SPARK_Search.js` (507) + `services/Search.js` (1,034)
...
- No `context.js`.
- `SearchService extends SparkService` takes the base URL **directly** (`this.endpoint = endpoint`, `services/Search.js:20`) — the REST service is **`plm-elastic-search`** (per platform direction).
- Every query builds an elastic query string/body and posts to this backend.
...
| File | Lines | Role |
|---|---|---|
| `schemas/SPARK_Search.graphqls` | 655 | the source SDL — ~48 queries, 1 mutation, ~50 result/value types, ~10 inputs |
| `resolvers/SPARK_Search.js` | 507 | ~43 query resolvers, 1 mutation, ~12 result-type field-resolver blocks |
| `services/Search.js` | 1,034 ⚠️ | ~80 elastic query-builder methods (suggestions, materials, samples, teams, templates, …) |
| **⚠️ Large file** | 1,034 | service read by section; resolver/SDL read in full |

Schema: **`schemas/SPARK_Search.graphqls` (655 lines)** — target schema in [03-schema.graphql](./03-schema.graphql)
translated from it (nullability from the SDL).

## 3. Import Graph
```
resolvers/SPARK_Search.js imports (to ENRICH elastic hits) from:
...
- domain calls `ctx.loaders.search.*` (the 🔴 search dependency seen across product/bom/measurement/packaging/ productDetails/claims/watchlist/workspace).
- In the target, those become calls to this subgraph's queries.
...
- in the workspace resolver**; `searchSPGSuggestions`/`searchUsersSuggestions`/`searchTeamsSuggestions` are declared in the SDL but have **no resolver in this snapshot** (confirm owner).
- Flag in Phase 3.
