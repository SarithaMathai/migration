...
| 2 | `searchProducts` (no resolver) vs `searchProductsES` (not in SDL) — which to keep | F01/C06 | Product Owner |
| 3 | Owner for `searchSPG/Users/TeamsSuggestions` (no resolver) + the 2 workspace-resolved suggestions | F01/C09 | Product Owner |
| 4 | Scope of A02 — expand all `JSON`-placeholder value types to concrete SDL types | A02 | Product Owner |
