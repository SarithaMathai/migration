...
- URL: https://confluence.target.com/spaces/PPDE/pages/1313880240/Federated+GraphQL+Breakdown+%E2%80%94+Bill+of+Materials+BOM
- Page id: 1313880240  (space: PPDE)
- Keep the existing title as-is.

Source: finalArtifacts/summary/bom/FederatedGqlBreakDown-bom.md
...
- IDs that look like codes or tickets (e.g. BOM-BE-B-01, SPIKE-04) stay as plain text —
...
Before writing, do a DRY RUN: confirm you resolved page id 1313880240 and its current title, and give a
...
Then ensure finalArtifacts/jira/confluence-page-map.csv has a row for bom pointing at this page
(header "Domain,Breakdown Page URL,FE Readiness Page URL"; create the file if missing; update the
existing bom row's Breakdown Page URL in place rather than adding a duplicate):
bom,https://confluence.target.com/spaces/PPDE/pages/1313880240/Federated+GraphQL+Breakdown+%E2%80%94+Bill+of+Materials+BOM,

---

Open page id 1313880240 and compare it section-by-section against
finalArtifacts/summary/bom/FederatedGqlBreakDown-bom.md. Confirm: same number of tables
