...
> **Home domains:** product (`plm-product`) + workspace (`plm-workspace`) · **Stub story:** `SPARK-PROD-G02` + `SPARK-WS-G02`/`G04`

> **Migrates (source resolvers → this case):** product **FR `components`** (`SPARK-PROD-G02`, the per-claim N+1
> ACL) + workspace **FR `counts`** (`SPARK-WS-G02`) and `products`/`productsCount`/`combinations` (`SPARK-WS-G04`).
...
### `WorkspaceV2.counts` (~85 ln, `SPARK-WS-G02`) — the product dashboard rollup
...
- `WorkspaceV2.products`/`productsCount`/`combinations` (`SPARK-WS-G04`) are the cross-subgraph list siblings.
