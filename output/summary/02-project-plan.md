...
|---|---|---|---|---|
| 1 | **Watchlist** | Wave 1 pilot — smallest isolated surface; proves flag flip + rollback | 12 | 3 |
| 2 | **Product Details** | Wave 2 — small, isolated, no shared blockers | 11 | 3 |
| 3 | **Measurement** | Wave 2 — parallel with Product Details | 15 | 4 |
| 4 | **Packaging** | Wave 2 — parallel with the other wave-2 domains | 18 | 5 |
| 5 | **BOM** | Wave 3 — high complexity, search-gated list views | 25 | 7 |
| 6 | **Claims** | Wave 3 — first cross-subgraph cutover (`spark-claims`) | 16 | 4 |
| 7 | **Product** | Wave 4 — largest surface, incremental slices, orchestrated writes last | 51 | 12 |
...
> Wave 1 pilot — smallest isolated surface; proves flag flip + rollback · **12 BE + 3 FE stories** · ≈ 31 BE + 8 FE nominal days · **Owners:** Backend BE-2 · Frontend FE-1. Stories in the same step are independent and parallelize; a FE story never starts before every BE story it depends on is delivered.

**Roadmap**

- **Backend (BE-2):** `B-01` → (9 in parallel: `B-02` … `G-03`) → `G-04` ‖ `G-05`
...
| 9 | 2 | 🟡 `G-02` — createdBy + updatedBy + workspaces + pa… | Backend · BE-2 | `B-01` — getWatchlistByIds | `G-04`, `G-05` | Yes |
...
| 12 | 3 | 🟢 `G-05` — WatchlistPartner.partner entity referen… | Backend · BE-2 | `G-02` — createdBy + updatedBy +… | None | Yes |
| 13 | 3 | 🟢 `WATCHLIST-FE-001` — Migrate watchlist reads | Frontend · FE-1 | `B-01` — getWatchlistByIds, `C-01` — getWatchlistByFilter | None | Yes |
| 14 | 4 | 🟢 `WATCHLIST-FE-002` — Migrate watchlist create and clone muta… | Frontend · FE-1 | `D-01` — createWatchlistEntries, `D-02` — cloneFilesForWatchlist | None | No |
| 15 | 5 | 🟡 `WATCHLIST-FE-003` — Migrate updateWatchlistEntries saga han… | Frontend · FE-1 | `E-01` — updateWatchlistEntries | None | No |
...
> Wave 2 — parallel with Product Details · **15 BE + 4 FE stories** · ≈ 38 BE + 16 FE nominal days · **Owners:** Backend BE-2 · Frontend FE-2. Stories in the same step are independent and parallelize; a FE story never starts before every BE story it depends on is delivered.

**Roadmap**

- **Backend (BE-2):** `B-01` → (12 in parallel: `B-02` … `G-02`) → `G-03` ‖ `G-04`
- **↓ unlocks frontend after backend step 2**
...
| 2 | 2 | 🟡 `B-02` — getUnitsOfMeasure · getThicknessUnitsOf… | Backend · BE-2 | `B-01` — getMeasurementByIds dat… | `G-04`, `MST-FE-001`, `MST-FE-003` | Yes |
...
| 15 | 3 | 🟢 `G-04` — SampleMeasurementSet.sample forward ref… | Backend · BE-2 | `B-02` — getUnitsOfMeasure · get… | None | Yes |
| 16 | 3 | 🟡 `MST-FE-001` — Migrate measurement reads and retire hu… | Frontend · FE-2 | `B-01` — getMeasurementByIds dat…, `B-02` — getUnitsOfMeasure · get… | None | Yes |
| 17 | 3 | 🟢 `MST-FE-003` — Migrate measurement master-data reads | Frontend · FE-2 | `B-02` — getUnitsOfMeasure · get… | None | Yes |
| 18 | 4 | 🟡 `MST-FE-002` — Migrate measurement list/search reads | Frontend · FE-2 | `C-01` — getMeasurements data fe…, `C-02` — getMeasurementsElastic … | None | No |
| 19 | 5 | 🟡 `MST-FE-004` — Migrate measurement mutations | Frontend · FE-2 | `D-02` — updateMeasurementAccess…, `D-06` — putSampleMeasurementSet…, `D-07` — deleteSampleMeasurement… | None | No |
...
> Wave 3 — high complexity, search-gated list views · **25 BE + 7 FE stories** · ≈ 79 BE + 38 FE nominal days · **Owners:** Backend BE-2 · Frontend FE-1. Stories in the same step are independent and parallelize; a FE story never starts before every BE story it depends on is delivered.

**Roadmap**

- **Backend (BE-2):** `B-01` → (17 in parallel: `A-04` … `G-10`) → (6 in parallel: `E-01` … `G-17`) → `G-16`
- **↓ unlocks frontend after backend step 2**
...
| 13 | 2 | 🟡 `G-01` — Bom field resolvers | Backend · BE-2 | None | `G-17` | Yes |
...
| 24 | 3 | 🟡 `G-17` — supplier entity references on material … | Backend · BE-2 | `G-01` — Bom field resolvers | `BOM-FE-007` | Yes |
| 25 | 3 | 🔴 `BOM-FE-001` — Statically expand BOM fragment factories | Frontend · FE-1 | None | `BOM-FE-002` | Yes |
| 26 | 3 | 🟢 `BOM-FE-004` — Migrate BOM master-data reads | Frontend · FE-1 | `B-05` — getBomMaterialTypes, `B-03` — getBomStatus · getBomBy…, `B-08` — getBomPackagingUnitOfMe… | None | Yes |
| 27 | 4 | 🟡 `G-16` — Test coverage & parity harness | Backend · BE-2 | `E-01` — updateBom — 3-step orch…, `G-08` — BomTrimMaterial field r…, `G-10` — Impression library-reso… | None | Yes |
| 28 | 4 | 🔴 `BOM-FE-002` — Migrate BOM core reads | Frontend · FE-1 | `B-01` — getBomByIds data fetcher, `B-03` — getBomStatus · getBomBy…, `BOM-FE-001` — Statically expand BOM f… | `BOM-FE-007` | Yes |
| 29 | 4 | 🟡 `BOM-FE-005` — Migrate BOM supplier reads | Frontend · FE-1 | `C-03` — getComboSupplierForBom …, `C-01` — getBomElastic data fetc… | None | Yes |
| 30 | 5 | 🟢 `BOM-FE-007` — Adopt BOM supplier entity references | Frontend · FE-1 | `G-17` — supplier entity referen…, `BOM-FE-002` — Migrate BOM core reads | None | No |
| 31 | 6 | 🔴 `BOM-FE-003` — Migrate BOM search and elastic reads | Frontend · FE-1 | `C-01` — getBomElastic data fetc…, `BOM-BE-S-03` | None | Yes |
| 32 | 6 | 🔴 `BOM-FE-006` — Migrate BOM mutations including updateB… | Frontend · FE-1 | `D-01` — addBom mutation, `D-02` — manageBomWorkspaces mut…, `BOM-BE-S-01` | None | Yes |
...
> Wave 3 — first cross-subgraph cutover (`spark-claims`) · **16 BE + 4 FE stories** · ≈ 48 BE + 22 FE nominal days · **Owners:** Backend BE-1 · Frontend FE-2. Stories in the same step are independent and parallelize; a FE story never starts before every BE story it depends on is delivered.

**Roadmap**

- **Backend (BE-1):** `B-01` → (13 in parallel: `B-02` … `G-06`) → `G-03` → `G-05`
...
|---|---|---|---|
| 1 | 🟢 `B-01` | 🟡 `CLAIM-FE-001` | — |
| 2 | 🟡 `B-02`, 🟡 `C-01`, 🟡 `C-02`, 🟡 `D-01`, 🟡 `D-02`, 🟡 `D-03`, 🟠 `E-01`, 🟡 `F-01`, 🟢 `F-02`, 🟡 `G-01`, 🟡 `G-02`, 🟡 `G-04`, 🟢 `G-06` | — | `E-01` → 🔬 SPIKE-01<br>`F-01` → ⛔ BLOCKED-BY product<br>`F-02` → ⛔ BLOCKED-BY product |
| 3 | 🟠 `G-03` | 🔴 `CLAIM-FE-002` | — |
| 4 | 🟡 `G-05` | 🟡 `CLAIM-FE-003` | — |
| 5 | — | 🔴 `CLAIM-FE-004` | — |
...
| 14 | 2 | 🟡 `G-04` — workspaces + ClaimSubstantiate.substant… | Backend · BE-1 | `B-01` — getClaims | None | Yes |
| 15 | 2 | 🟢 `G-06` — Shared value-type alignment | Backend · BE-1 | `B-01` — getClaims | `G-03` | Yes |
| 16 | 3 | 🟠 `G-03` — product + parentDetails | Backend · BE-1 | `B-01` — getClaims, `G-06` — Shared value-type align… | `G-05` | Yes |
...
| 18 | 4 | 🟡 `G-05` — Tests, parity harness | Backend · BE-1 | `B-01` — getClaims, `E-01` — updateClaim, `G-02` — createdBy + updatedBy +…, `G-03` — product + parentDetails | None | Yes |
| 19 | 4 | 🟡 `CLAIM-FE-003` — Migrate claim simple mutations and expo… | Frontend · FE-2 | `D-01` — createClaim, `D-02` — bulkUpdateClaim, `D-03` — requestClaimExport · lo… | None | Yes |
| 20 | 5 | 🔴 `CLAIM-FE-004` — Migrate updateClaim multi-step write ha… | Frontend · FE-2 | `E-01` — updateClaim | None | No |
...
> Wave 4 — largest surface, incremental slices, orchestrated writes last · **51 BE + 12 FE stories** · ≈ 182 BE + 84 FE nominal days · **Owners:** Backend BE-1 · Frontend FE-1 + FE-2. Stories in the same step are independent and parallelize; a FE story never starts before every BE story it depends on is delivered.

**Roadmap**

- **Backend (BE-1):** `B-01` → (36 in parallel: `B-02` … `G-15`) → (12 in parallel: `E-04` … `G-17`) → `F-09` ‖ `F-11`
- **↓ unlocks frontend after backend step 2**
...
|---|---|---|---|
| 1 | 🟢 `B-01` | — | — |
| 2 | 🟡 `B-02`, 🟡 `B-06`, 🟢 `B-10`, 🟠 `C-01`, 🟡 `C-02`, 🟡 `C-03`, 🟡 `C-04`, 🟡 `C-05`, 🟡 `D-01`, 🟡 `D-02`, 🟡 `D-03`, 🟡 `D-04`, 🟡 `D-05`, 🟡 `D-06`, 🟡 `D-10`, 🟡 `D-14`, 🟢 `D-18`, 🔴 `E-01`, 🟠 `E-02`, 🔴 `E-03`, 🟡 `F-13`, 🟢 `F-14`, 🔴 `G-01`, 🔴 `G-02`, 🟡 `G-04`, 🟡 `G-05`, 🟡 `G-06`, 🟡 `G-07`, 🟡 `G-08`, 🟡 `G-09`, 🟡 `G-10`, 🟡 `G-11-1`, 🟡 `G-11-2`, 🟢 `G-12`, 🟡 `G-13`, 🟡 `G-15` | — | `C-01` → 🔬 SPIKE-06a<br>`D-01` → 🔬 SPIKE-06b<br>`D-02` → 🔬 SPIKE-06b<br>`D-04` → 🔬 SPIKE-06b<br>`E-01` → 🔬 SPIKE-03<br>`E-02` → 🔬 SPIKE-01<br>`E-03` → 🔬 SPIKE-02<br>`G-07` → 🔬 SPIKE-04<br>`G-11-1` → 🔬 SPIKE-04 |
| 3 | 🔴 `E-04`, 🟡 `F-01`, 🟡 `F-02`, 🟡 `F-03`, 🟡 `F-04`, 🟡 `F-05`, 🟡 `F-07`, 🟢 `F-10`, 🟢 `F-12`, 🟠 `G-03`, 🟠 `G-16`, 🟡 `G-17` | 🔴 `PRODUCT-FE-001`, 🟡 `PRODUCT-FE-004` | `E-04` → 🔬 SPIKE-02<br>`F-01` → ⛔ BLOCKED-BY attachment domain (⛔ cross-subgraph — does not ship until plm-attachment is live)<br>`F-02` → ⛔ BLOCKED-BY discussion domain (⛔ cross-subgraph)<br>`F-03` → ⛔ BLOCKED-BY sample domain (⛔ cross-subgraph)<br>`F-05` → ⛔ BLOCKED-BY claim domain (⛔ cross-subgraph)<br>`F-07` → ⛔ BLOCKED-BY construction domain (⛔ cross-subgraph) |
...
| 22 | 2 | 🟡 `F-13` — Product entity fetcher  for cross-subgr… | Backend · BE-1 | `B-01` — getProduct | `F-10` | Yes |
| 23 | 2 | 🟢 `F-14` — Cross-subgraph contract alignment | Backend · BE-1 | None | `F-10`, `PRODUCT-FE-012` | Yes |
| 24 | 2 | 🔴 `G-01` — Product.attachmentsWithMetaData | Backend · BE-1 | None | `G-03`, `G-16`, `G-17` | Yes |
| 25 | 2 | 🔴 `G-02` — Product.components | Backend · BE-1 | None | `G-16` | Yes |
| 26 | 2 | 🟡 `G-04` — ProductsCategories.categories  + Dopple… | Backend · BE-1 | None | None | Yes |
| 27 | 2 | 🟡 `G-05` — Product.samples + sampleIds + elasticSa… | Backend · BE-1 | None | None | Yes |
| 28 | 2 | 🟡 `G-06` — Product.teams + discussionsV2 + discuss… | Backend · BE-1 | None | None | Yes |
| 29 | 2 | 🟡 `G-07` — Product.vendorAttributes + businessPart… | Backend · BE-1 | None | None | Yes |
| 30 | 2 | 🟡 `G-08` — Product.measurementSets + claims + bom … | Backend · BE-1 | None | None | Yes |
| 31 | 2 | 🟡 `G-09` — Product.productWorkspaceAttributes + pr… | Backend · BE-1 | None | None | Yes |
| 32 | 2 | 🟡 `G-10` — Product.ancestryProducts + rating + res… | Backend · BE-1 | None | None | Yes |
| 33 | 2 | 🟡 `G-11-1` — Product.notRemovablePartnerIds + notRem… | Backend · BE-1 | None | None | Yes |
| 34 | 2 | 🟡 `G-11-2` — Product.associateProductsAsks + Product… | Backend · BE-1 | None | None | Yes |
| 35 | 2 | 🟢 `G-12` — Product.division **bug fix** · Simple u… | Backend · BE-1 | None | None | Yes |
| 36 | 2 | 🟡 `G-13` — IG/tag/tcin/spg + template trivial-fiel… | Backend · BE-1 | None | None | Yes |
| 37 | 2 | 🟡 `G-15` — Port product utils to Kotlin | Backend · BE-1 | None | None | Yes |
| 38 | 3 | 🔴 `E-04` — getProductTechPackBulkCountV1 | Backend · BE-1 | `E-03` — getProductTechPackCount… | `PRODUCT-FE-010` | Yes |
| 39 | 3 | 🟡 `F-01` — ResourcesCount.productAttachments + dis… | Backend · BE-1 | `E-03` — getProductTechPackCount… | `F-09` | Yes |
| 40 | 3 | 🟡 `F-02` — ResourcesCount.discussions | Backend · BE-1 | `E-03` — getProductTechPackCount… | `F-09` | Yes |
| 41 | 3 | 🟡 `F-03` — ResourcesCount.sample | Backend · BE-1 | `E-03` — getProductTechPackCount… | `F-09` | Yes |
| 42 | 3 | 🟡 `F-04` — ResourcesCount.measurementSets · Resour… | Backend · BE-1 | `E-03` — getProductTechPackCount… | `F-09` | Yes |
| 43 | 3 | 🟡 `F-05` — ResourcesCount.claims | Backend · BE-1 | `E-03` — getProductTechPackCount… | `F-09` | Yes |
| 44 | 3 | 🟡 `F-07` — ResourcesCount.constructions | Backend · BE-1 | `E-03` — getProductTechPackCount… | `F-09` | Yes |
| 45 | 3 | 🟢 `F-10` — Hive Gateway supergraph composition | Backend · BE-1 | `F-13` — Product entity fetcher …, `F-14` — Cross-subgraph contract… | `F-11` | Yes |
| 46 | 3 | 🟢 `F-12` — Deferred partner-wrapper decision | Backend · BE-1 | `E-01` — productBusinessPartnerA… | None | Yes |
| 47 | 3 | 🟠 `G-03` — Product.attachments + attachmentsV3 + a… | Backend · BE-1 | `G-01` — Product.attachmentsWith… | None | Yes |
| 48 | 3 | 🟠 `G-16` — Test coverage, parity harness, load & c… | Backend · BE-1 | `C-01` — getProducts two-stage h…, `E-01` — productBusinessPartnerA…, `E-03` — getProductTechPackCount…, `G-01` — Product.attachmentsWith…, `G-02` — Product.components | None | Yes |
| 49 | 3 | 🟡 `G-17` — Entity references on partner/lineage va… | Backend · BE-1 | `G-01` — Product.attachmentsWith… | None | Yes |
| 50 | 3 | 🔴 `PRODUCT-FE-001` — Migrate getProduct documents in product… | Frontend · FE-1 + FE-2 | `B-01` — getProduct | `PRODUCT-FE-002`, `PRODUCT-FE-008` | Yes |
| 51 | 3 | 🟡 `PRODUCT-FE-004` — Migrate product status and workspace-co… | Frontend · FE-1 + FE-2 | `B-02` — getProductsByIds · getP… | None | Yes |
| 52 | 4 | 🟢 `F-09` — Retire the TechPack aggregation facade | Backend · BE-1 | `F-01` — ResourcesCount.productA…, `F-02` — ResourcesCount.discussi…, `F-03` — ResourcesCount.sample, `F-04` — ResourcesCount.measurem…, `F-05` — ResourcesCount.claims, `F-07` — ResourcesCount.construc… | None | Yes |
| 53 | 4 | 🟢 `F-11` — Platform stub verification | Backend · BE-1 | `F-10` — Hive Gateway supergraph… | None | Yes |
| 54 | 4 | 🟡 `PRODUCT-FE-002` — Migrate shared-library getProduct consu… | Frontend · FE-1 + FE-2 | `B-01` — getProduct, `B-02` — getProductsByIds · getP…, `PRODUCT-FE-001` — Migrate getProduct docu… | None | Yes |
| 55 | 4 | 🟡 `PRODUCT-FE-005` — Migrate template library and categories… | Frontend · FE-1 + FE-2 | `C-02` — getProductTemplates, `C-03` — getCategories | None | Yes |
| 56 | 5 | 🟡 `PRODUCT-FE-006` — Migrate product rules administration | Frontend · FE-1 + FE-2 | `B-06` — getProductTemplateById …, `B-10` — getProductDeptRules · g…, `C-05` — searchProductRules, `D-14` — unlinkProduct · addProd… | None | Yes |
| 57 | 5 | 🟡 `PRODUCT-FE-007` — Migrate simple product mutations | Frontend · FE-1 + FE-2 | `D-01` — addProduct, `D-02` — addProducts, `D-03` — bulkUpdateProducts, `D-04` — updateProduct, `D-05` — carryForwardProduct, `D-10` — updateViewToggle · upda…, `D-14` — unlinkProduct · addProd… | None | Yes |
| 58 | 5 | 🟡 `PRODUCT-FE-008` — Migrate team and partner assignment mut… | Frontend · FE-1 + FE-2 | `D-06` — addTeamsToProduct 🔀 Col…, `D-10` — updateViewToggle · upda…, `PRODUCT-FE-001` — Migrate getProduct docu… | None | Yes |
| 59 | 6 | 🟡 `PRODUCT-FE-010` — Migrate TechPack count queries | Frontend · FE-1 + FE-2 | `E-03` — getProductTechPackCount…, `E-04` — getProductTechPackBulkC… | None | Yes |
| 60 | 6 | 🟡 `PRODUCT-FE-011` — Migrate component status rollups | Frontend · FE-1 + FE-2 | `B-01` — getProduct, `D-18` — updateComponentStatus, `E-02` — updateComponentStatuses | None | Yes |
| 61 | 6 | 🟡 `PRODUCT-FE-012` — Verify fragment type-conditions, __type… | Frontend · FE-1 + FE-2 | `F-14` — Cross-subgraph contract… | None | Yes |
| 62 | 7 | 🔴 `PRODUCT-FE-003` — Migrate product list and bulk reads | Frontend · FE-1 + FE-2 | `PRODUCT-BE-S-02`, `B-02` — getProductsByIds · getP… | None | Yes |
| 63 | 7 | 🔴 `PRODUCT-FE-009` — Migrate partner drop/undrop orchestrati… | Frontend · FE-1 + FE-2 | `PRODUCT-BE-S-03`, `D-06` — addTeamsToProduct 🔀 Col… | None | Yes |

**Backend critical path:** `B-01` → `E-03` → `F-03` → `F-09`.
...
| 5 | 2 | 🟢 `G-01` — Impression field resolvers · attachment… | Backend · BE-1 | `B-01` — searchImpressionsByProd… | None | Yes |
