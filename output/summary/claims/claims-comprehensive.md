# Claims — Comprehensive Migration Documentation

> **Domain:** `claims` · **Target DGS:** `spark-claims (separate)` · **Generated:** 2026-07-19
> **Confluence location:** *Federation Graph Migration ▸ Domains ▸ claims*
...
|---|---|---|
| `CLAIM-BE-E-01` updateClaim (proxy ACL + workspace + body) | see story | E-01-1 body PUT + workspace call · E-01-2 ACL proxy + orchestration |
...
> **Note — DGS Module Init (this PR only):** Creates `claims.graphqls` (federation v2.3 header, scalars, owned types with `@key`, external stubs), registers scalars in `ScalarConfig.kt`, and wires the service and Feign client. Full type list: be-03-schema.graphql.
...
| **Phase** | G |
| **Depends on** | B-01 |
| **EXT** | 🟡 `workspaceV2` · 🟡 `userAttributes` |
...
| **Phase** | H |
| **Depends on** | B-01 |
| **Blocked by** | product (`PRODUCT-BE-F-14`, product-side stub alignment; also waits on the `Product` entity existing, plm-product Phase A) |
...
| **Phase** | H |
| **Depends on** | B-01 |
| **Blocked by** | product (`PRODUCT-BE-E-03`, TechPack facade; also `PRODUCT-BE-F-14` contract alignment) |
...
|---|---|---|---|---|
| `CLAIM-BE-B-01` | `getClaims(parentHumanId, claimHumanIds, partnerIds)` | B | 🟢 Low | — |
| `CLAIM-BE-B-02` | `getClaimByIds(claimHumanIds)` | B | 🟢 Low | B-01 |
| `CLAIM-BE-B-03` | `getCommunicationChannels` (cacheable) | B | 🟢 Low | B-01 |
| `CLAIM-BE-B-04` | `getAllClaimsAbout` (cacheable) | B | 🟢 Low | B-01 |
| `CLAIM-BE-B-05` | `getClaimExports` | B | 🟢 Low | B-01 |
| `CLAIM-BE-C-01` | `searchGuestFacing(queryParam)` | C | 🟡 Medium | B-01 |
| `CLAIM-BE-C-02` | `getClaimsElastic(parentHumanId)` | C | 🟡 Medium | B-01 |
| `CLAIM-BE-D-01` | `createClaim` | D | 🟡 Medium | B-01 |
| `CLAIM-BE-D-02` | `bulkUpdateClaim` | D | 🟡 Medium | B-01 |
| `CLAIM-BE-D-03` | `requestClaimExport` | D | 🟢 Low | B-01 |
| `CLAIM-BE-D-04` | `lockClaim` | D | 🟢 Low | B-01 |
| `CLAIM-BE-D-05` | `unlockClaim` | D | 🟢 Low | B-01 |
| `CLAIM-BE-E-01` | `updateClaim` (proxy ACL + multi-step write) | E | 🟠 High | B-01 |
| `CLAIM-BE-H-01` | `Product.claims` (federation contribution) | H | 🟡 Medium | B-01 |
| `CLAIM-BE-H-02` | `ResourcesCount.claims` (TechPack — claims side of PRODUCT-BE-H-04) | H | 🟢 Low | B-01 |
| `CLAIM-BE-G-01` | `access` + `currentUserPermissions` + `participantDetails` | G | 🟡 Medium | B-01 |
| `CLAIM-BE-G-02` | `createdBy` + `updatedBy` + `businessPartner` + `designPartner` | G | 🟡 Medium | B-01 |
| `CLAIM-BE-G-03` | `product` + `parentDetails` (otherClaimBps / systemTeams / droppedPartnerIds) | G | 🟠 High | B-01, G-06 |
| `CLAIM-BE-G-04` | `workspaces` + `ClaimSubstantiate.substantiatedBy` + `ClaimDetails.claimName` | G | 🟡 Medium | B-01 |
| `CLAIM-BE-G-06` | Shared value-type alignment (`@shareable` instead of entity stubs) | G | 🟢 Low | B-01 |