# Federated GraphQL — Migration Overview · Selected Modules

> **Scoped overview** — a rollup of **8 selected modules** (Product, Bill of Materials (BOM), Claims, Product Details, Packaging, Watchlist, Measurement, Impression). The full-program overview is the untouched `Federated+Graphql+Stories+-+BreakDown` page.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | **Product** | `plm-product (host)` | **XXL** | **69** | 5 | 4 | 27 | 33 | **12** | 66–102d | `FederatedGqlBreakDown-product` |
| 2 | **Bill of Materials (BOM)** | `plm-product (co-located)` | **XL** | **37** | 1 | 2 | 13 | 21 | **7** | 29–46d | `FederatedGqlBreakDown-bom` |
| 3 | **Claims** | `spark-claims (separate)` | **L** | **20** | 0 | 2 | 8 | 10 | **4** | 17–27d | `FederatedGqlBreakDown-claims` |
| 4 | **Product Details** | `plm-product (co-located)` | **M** | **12** | 0 | 1 | 6 | 5 | **3** | 8–12d | `FederatedGqlBreakDown-productDetails` |
| 5 | **Packaging** | `plm-product (co-located)` | **L** | **23** | 0 | 2 | 8 | 13 | **5** | 21–33d | `FederatedGqlBreakDown-packaging` |
| 6 | **Watchlist** | `plm-product (co-located)` | **M** | **13** | 0 | 1 | 5 | 7 | **3** | 7–10d | `FederatedGqlBreakDown-watchlist` |
| 7 | **Measurement** | `plm-product (co-located)` | **M** | **30** | 0 | 1 | 5 | 24 | **4** | 12–19d | `FederatedGqlBreakDown-measurement` |
| 8 | **Impression** | `plm-product (co-located)` | **XS** | **7** | 0 | 0 | 2 | 5 | **2** | 3–5d | `FederatedGqlBreakDown-impression` |
...
**Decision to make:** Ratify the assembly pattern and each domain’s contribution. Draft ADR-015 proposes **facade-then-federate** (ADR-015 Option B; the catalogue label is "Option D (hybrid)"): a frozen aggregation facade serves all 11 fields day 1 (`E-03`/`E-04`); each domain re-homes its slice as its subgraph ships (`H-01`–`H-05` + the co-located `F-04`/`F-06`/`F-08`, `extend type ResourcesCount`); the facade retires last (`F-09`).
