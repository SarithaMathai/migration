# Bill of Materials (BOM) — Migration to the `plm-product` DGS (PO view)

> 🏷️ **Tags:** `dgs-migration` · `po-page` · `domain-bom` · `pipeline-v2`  —  **Confluence location:** *Federation Graph Migration ▸ Domains ▸ bom*
...
> Create tickets from `../finalOutput/jira/bom.csv`. Effort is **AI-estimated — confirm in refinement.**
...
> 🔬 Open decisions marked **Spike** are tracked as real, estimable Phase 0 stories — see *Phase 0 — Spikes* in `../finalOutput/bom/be-04-stories.md` and `FederatedGqlBreakDown-bom.md` for the full write-up (unknowns, candidate patterns, examples).

## Migration approach (summary)

**B** Core Reads; **C** Search & Listing; **D** Mutations (simple); **E** Complex (`updateBom`); **F** Federation Contributions; **G** Field Resolvers & Tests. Full detail: `be-03-schema-analysis.md §Migration Approach`.
...
Two stories in this domain were broken into **M-size (≤5 day) sub-tasks** in Jira.

| Parent milestone | Original size | Sub-tasks |
|---|---|---|
| `BOM-BE-E-01` updateBom — 3-step orchestrated write | Very High | E-01-1 workspace-assoc + body PUT · E-01-2 permissions PUT + rollback framework |
| `BOM-BE-G-08` BomTrimMaterial field resolvers (dispatcher + 7 types) | High | G-08-1 dispatcher scaffold + type resolution · G-08-2 7 TrimMaterial field resolvers |

> `BOM-BE-E-01` is also the home stub for the **non-atomic-write-saga** complex case. See `complexStories/non-atomic-write-saga/`.

> In Jira, sub-tasks appear **nested under** their parent story. Sprint capacity: each sub-task = M (3–5 days).

---
*PO review assembled from the `bom` analysis. Jira tickets: `finalOutput/jira/bom.csv`. Full engineering detail: `bom-comprehensive.md` (same folder).*