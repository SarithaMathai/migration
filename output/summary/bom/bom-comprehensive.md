# Bill of Materials (BOM) — Comprehensive Migration Documentation

> **Domain:** `bom` · **Target DGS:** `plm-product (co-located)` · **Generated:** 2026-07-19
> **Confluence location:** *Federation Graph Migration ▸ Domains ▸ bom*
...
- [All Stories — Detailed Engineering Breakdown](#all-stories--detailed-engineering-breakdown)
  - [Phase A — Foundation & Type Resolvers](#phase-a--foundation-type-resolvers)
...
> `be-04-stories.md` for the full write-up of each. The rest were resolved directly.
...
|---|---|---|
| `BOM-BE-E-01` updateBom — 3-step orchestrated write | see story | E-01-1 workspace-assoc + body PUT · E-01-2 permissions PUT + rollback framework · Complex case: `non-atomic-write-saga` |

### High stories (split into M-size sub-tasks)

| Parent milestone | Why split | Sub-tasks |
|---|---|---|
| `BOM-BE-G-08` BomTrimMaterial field resolvers (7 types + dispatcher) | see story | G-08-1 dispatcher scaffold + type resolution · G-08-2 7 TrimMaterial field resolvers |
| `BOM-BE-A-05` Shared CI conformance gate + code→type registry | see story | A-05-1 SDL↔enum↔fixture conformance library · A-05-2 code→type registry seeding |
...
be-04-po-summary.md for AI-estimated ranges and sprint sequencing.
...
### Phase A — Foundation & Type Resolvers
...
| **Phase** | A |
| **Depends on** | A-04 |
| **Blocks** | every future PR touching a polymorphic dispatch site (this domain and, later phase, sample/search) |
...
| **Phase** | G |
| **Depends on** | — |
| **EXT** | 🟡 `combination` · 🔵 `tag` · 🟡 `materialHub` |
...
- **In summary:** `updateBom` today does three separate write calls in a row — save the workspace links,
...
> **Prior art, don't start from zero:** `../../complexStories/non-atomic-write-saga/00-overview.md`
...
2. If (a) or (b), the per-step compensation (specifically: how to undo the workspace-assoc step) is written down concretely enough for `E-01` to implement without re-deriving it
3. `E-01`'s "Chosen failure strategy" line is updated from "PO decision E-01" to the concrete choice
4. If a shared `WriteSaga` (prior art above) is adopted, note that `E-01` becomes the first *production* wiring of that shared component (`non-atomic-write-saga` is the design; `E-01` is an implementation of it)
...
- **In summary:** BOM's material field resolvers (trim, wash, fabric, fabric-spec, combination, and the
...
1. A rollout order for hub/trim/wash/fabric/combination is agreed with Platform + each sibling team and recorded here
2. The order is reflected in the sprint sequencing table in `be-04-po-summary.md`
3. `B-01`'s DGS-module-init checklist and each affected `G0x` story note which siblings are expected to be `{id}`-only at BOM's launch date
...
- **In summary:** today, when a caller sends `nestedSearchFilters` to `searchMaterialsBom`, the gateway
...
3. If moving to structured, `C-02`'s acceptance criteria #2 (today: "same `nestedSearchFilters[i].*` keys as today") is updated to describe the agreed structured shape instead
...
|---|---|---|---|---|
| `BOM-BE-S-01` | `updateBom` failure-strategy spike | S | 🟡 Medium | — |
| `BOM-BE-S-02` | Federation rollout-order spike (material sibling DGS) | S | 🟢 Low | — |
| `BOM-BE-S-03` | `searchMaterialsBom` request-shape spike | S | 🟢 Low | — |
| `BOM-BE-A-04` | `@DgsTypeResolver` for the 2 BOM interfaces | A | 🟡 Medium | — |
| `BOM-BE-A-05` | Shared CI conformance gate + `code → type` registry (`SPIKE-05`) | A | 🟡 Medium | A-04 |
| `BOM-BE-B-01` | `getBomByIds` data fetcher | B | 🟢 Low | — |
| `BOM-BE-B-03` | `getBomStatus` (cacheable master data) | B | 🟢 Low | — |
| `BOM-BE-B-04` | `getBomByParentId` data fetcher | B | 🟢 Low | — |
| `BOM-BE-B-05` | `getBomMaterialTypes` (merge with Material Hub) | B | 🟡 Medium | — |
| `BOM-BE-B-06` | `getBomPackagingMaterialTypes` (cacheable) | B | 🟢 Low | — |
| `BOM-BE-B-07` | `getBomPackagingSubstrates` (cacheable) | B | 🟢 Low | — |
| `BOM-BE-B-08` | `getBomPackagingUnitOfMeasure` (cacheable) | B | 🟢 Low | — |
| `BOM-BE-C-01` | `getBomElastic` data fetcher | C | 🟢 Low | — |
| `BOM-BE-C-02` | `searchMaterialsBom` data fetcher | C | 🟡 Medium | S-03 |
| `BOM-BE-C-03` | `getComboSupplierForBom` data fetcher | C | 🟡 Medium | — |
| `BOM-BE-C-04` | `getValidTrimSuppliersForBom` data fetcher | C | 🟢 Low | — |
| `BOM-BE-C-05` | `getValidRawMaterialSuppliersForBom` data fetcher | C | 🟢 Low | — |
| `BOM-BE-D-01` | `addBom` mutation | D | 🟡 Medium | — |
| `BOM-BE-D-02` | `manageBomWorkspaces` mutation | D | 🟢 Low | — |
| `BOM-BE-D-03` | `lockBom` mutation | D | 🟢 Low | — |
| `BOM-BE-D-04` | `unlockBom` mutation | D | 🟢 Low | — |
| `BOM-BE-D-05` | `updateBomComponentStatus` mutation | D | 🟢 Low | — |
| `BOM-BE-E-01` | `updateBom` — 3-step orchestrated write | E | 🔴 Very High | D-02, S-01 |
| `BOM-BE-F-01` | Implement `Product.productBoms` / `boms` / `packagingBoms` (internal) | F | 🟡 Medium | — |
| `BOM-BE-F-02` | Fill `ResourcesCount.bomsCount` (internal) | F | 🟢 Low | — |
| `BOM-BE-G-01` | `Bom` field resolvers (9 shared fields) | G | 🟡 Medium | — |
| `BOM-BE-G-03` | `BomMaterial` field resolvers (8 fields) | G | 🟡 Medium | — |
| `BOM-BE-G-04` | `BomPackagingMaterial` field resolvers (2 fields) | G | 🟢 Low | — |
| `BOM-BE-G-05` | `BomFabricMaterial` field resolvers (4 fields) | G | 🟢 Low | — |
| `BOM-BE-G-06` | `BomFabricSpecMaterial` field resolvers (4 fields) | G | 🟢 Low | — |
| `BOM-BE-G-07` | `BomCombinationMaterial` field resolvers (4 fields) | G | 🟢 Low | — |
| `BOM-BE-G-08` | `BomTrimMaterial` field resolvers (7 fields) — trim size dispatchers | G | 🟠 High | — |
| `BOM-BE-G-09` | `BomWashMaterial` field resolvers (4 fields) | G | 🟢 Low | — |
| `BOM-BE-G-10` | Impression library-resource resolution (shared internal/external branch + `MaterialDataLoader`) | G | 🟠 High | — |
| `BOM-BE-G-11` | `BomFabricLibraryImpressionDetails.libraryResource` | G | 🟡 Medium | G-10 |
| `BOM-BE-G-12` | `BomTrimLibraryImpressionDetails` field resolvers (3 fields) | G | 🟡 Medium | G-10 |
| `BOM-BE-G-13` | `BomTrimZipperLibraryImpressionDetails` field resolvers (3 colors) | G | 🟢 Low | G-10 |
| `BOM-BE-G-14` | Trivial pass-through fields (bundle) | G | 🟢 Low | — |
| `BOM-BE-G-15` | `BomMaterialSearchResult` field resolvers (5 fields) | G | 🟡 Medium | C-02 |
| `BOM-BE-G-17` | `supplier` entity references on material rows (recommended, PO-gated) | G | 🟡 Medium | G-01 |