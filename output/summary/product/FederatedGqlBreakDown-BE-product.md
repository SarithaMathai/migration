...
| Slot | 👤 BE-1 | 👤 BE-2 |
|---|---|---|
| 1 | 🟢 `B-01` (1–2d) | ⏳ after `B-01` → 🔴 `E-01` (7–12d) 🔬 |
...
| 🔷 `PRODUCT-BE-B-02`<br>`getProductsByIds` · `getProductStatus` · `getProductVersions` · `getCopyStatus` | 🟡 Medium `M` | Query | — | **Grouped XS story —** combines former `B-03`, `B-04`, `B-05` (one PR train)<br>**Intent —** Looks up several products at once by their ids; Returns the list of possible product statuses (dropdown options); Lists the saved versions of a product; Tells you whether a product copy is still in progress or done<br>**Today —** getByIdList GET ${v1}?productId={csv}&page=0&size=10000&sort=createdDate,desc; primes getByID. ; getStatus master status list. ; getVersions GET…<br>**Done when:**<br>• `getProductsByIds`: returns paged products for ids<br>• `getProductsByIds`: primes single-id loader<br>• `getProductStatus`: returns statuses<br>• `getProductStatus`: cached<br>• `getProductVersions`: returns versions<br>• `getCopyStatus`: returns copy status |
| 🔷 `PRODUCT-BE-B-06`<br>`getProductTemplateById` · `getProductRules` · `getProductRulesById` · `getAllAvailableRules` | 🟡 Medium `M` | Query | — | **Grouped XS story —** combines former `B-07`, `B-08`, `B-09` (one PR train)<br>**Intent —** Looks up a product template by id; Returns the product business rules; Looks up one business rule by id; Lists all the rules that are available to apply<br>**Today —** getByID → response \\|\\| {} (empty object on miss — preserve). ; getAllRules GET $… → content. ; getRuleById GET $… ; getAvailableRules GET …/spark_rules/v1/rules<br>**Done when:**<br>• `getProductTemplateById`: returns product or empty object (not null)<br>• `getProductRules`: returns rules content<br>• `getProductRulesById`: returns rule<br>• `getAllAvailableRules`: returns available rules |
| 🔷 `PRODUCT-BE-B-10`<br>`getProductDeptRules` · `getProductBPRules` | 🟢 Low `XS` | Query<br>Calls: `ruleLibrary` | — | **Grouped XS story —** combines former `B-11` (one PR train)<br>**Intent —** Returns the department-level rules for given products; Returns the business-partner-level rules for given products<br>**Today —** flag fork USE_NEW_RULES_API ? ruleLibrary.searchRuleLibrary : product.searchProductDeptRules GET …/spark_rules/v1/search?productIds=&departmentIds=&activeOnly=. PO…<br>**Done when:**<br>• `getProductDeptRules`: default `activeOnly=true`<br>• `getProductDeptRules`: flag selects the correct backend<br>• `getProductBPRules`: flag fork honored; BP filter applied |
...
| 🔶 `PRODUCT-BE-D-06`<br>`addTeamsToProduct` 🔀 Collab Canvas · `addBusinessPartnersToProductWithType` 🔀 Collab Canvas · `removeProductResources` · `updateBusinessPartnerStatuses` | 🟡 Medium `M` | Mutation | — | **Grouped XS story —** combines former `D-07`, `D-08`, `D-09` (one PR train)<br>**Intent —** Adds teams (and their partners) to a product; Adds business partners to a product with a given type; Removes resources (links) from a product; Updates the status of business partners on a product<br>**Today —** POST ${v1}/{productId}/resources/bulk + manage_workspace_teams. ; POST ${v1}/{productId}/partners-add/bulk; success = response has product_id and no status_code…<br>**Done when:**<br>• `addTeamsToProduct` 🔀 Collab Canvas: adds teams + new partners + workspace links<br>• `addTeamsToProduct` 🔀 Collab Canvas: partner-add failure exits early with a thrown typed error (today `return new Error(...)` — standardized per ADR-011 §4 pin-down 4, accepted deviation); teams are not added after a failed partner add (legacy order preserved)<br>• `addBusinessPartnersToProductWithType` 🔀 Collab Canvas: adds partners with type<br>• `addBusinessPartnersToProductWithType` 🔀 Collab Canvas: failure throws a typed `DgsException` instead of `return new Error(...)` (accepted parity deviation, ADR-011 §4 pin-down 4)<br>• `removeProductResources`: removes resources<br>• `updateBusinessPartnerStatuses`: updates partner statuses |
| 🔶 `PRODUCT-BE-D-10`<br>`updateViewToggle` · `updateWorkspaceAttributes` 🔀 Collab Canvas · `updateProductTeamsWorkspaceContext` · `linkProduct` | 🟡 Medium `M` | Mutation | — | **Grouped XS story —** combines former `D-11`, `D-12`, `D-13` (one PR train)<br>**Intent —** Toggles whether a product is hidden; Updates a product's workspace attributes; Adds or removes team↔workspace pairings on a product; Links a parent and child product together<br>**Today —** PUT ${v1} view toggle. ; PUT ${v1}/{productId} workspace attrs. ; PUT team-workspace add/remove. ; PUT link — throws on backend error (only mutation that does)<br>**Done when:**<br>• `updateViewToggle`: toggles hidden<br>• `updateWorkspaceAttributes` 🔀 Collab Canvas: updates workspace attrs<br>• `updateProductTeamsWorkspaceContext`: adds/removes team-workspace pairs<br>• `linkProduct`: links parent/child<br>• `linkProduct`: backend error → exception (not null) |
| 🔶 `PRODUCT-BE-D-14`<br>`unlinkProduct` · `addProductRule` · `updateProductRule` · `deleteProductRule` | 🟡 Medium `M` | Mutation | — | **Grouped XS story —** combines former `D-15`, `D-16`, `D-17` (one PR train)<br>**Intent —** Unlinks a parent and child product; Creates a product rule; Updates a product rule; Deletes a product rule<br>**Today —** PUT unlink. ; POST …/spark_rules/v1. ; PUT …/spark_rules/v1/{id}. ; DELETE …/spark_rules/v1/{id} → Boolean<br>**Done when:**<br>• `unlinkProduct`: unlinks parent/child<br>• `addProductRule`: creates rule<br>• `updateProductRule`: updates rule<br>• `deleteProductRule`: deletes; returns Boolean |
...
| 🔸 `PRODUCT-BE-F-04`<br>`ResourcesCount.measurementSets` · `ResourcesCount.productBoms` + `packagingBoms` + `boms` · `ResourcesCount.watchlists` | 🟡 Medium `M` | Field Resolver | E-03 | **Grouped XS story —** combines former `F-06`, `F-08` (one PR train)<br>**Intent —** Fills in the product's measurement-set count — answered in-process by the co-located Measurement code; Fills in the product's BOM counts — answered in-process by the co-located BOM code; Fills in the product's watchlist count — answered in-process by the co-located Watchlist code<br>**Done when:**<br>• `ResourcesCount.measurementSets`: `measurementSets` resolves in-process; no gateway hop; parity vs facade<br>• `ResourcesCount.productBoms` + `packagingBoms` + `boms`: `productBoms`/`packagingBoms`/`boms` resolve in-process; no gateway hop; parity vs facade<br>• `ResourcesCount.watchlists`: `watchlists` resolves in-process; no gateway hop; parity vs facade |
...
| 🔸 `PRODUCT-BE-G-12`<br>`Product.division` **bug fix** · Simple user/status fields + trivial pass-throughs | 🟢 Low `XS` | Field Resolver<br>Calls: `ig`, `userAttributes` | — | **Grouped XS story —** combines former `G-14` (one PR train)<br>**Intent —** Fix the wrong-loader bug so `division` returns the product's actual division; Resolve simple people / status fields and trivial pass-throughs<br>**Today —** Product.division is supposed to return the product's division - (a specific org unit). - Instead, the source code has a copy-paste-style mistake: it calls…<br>**Done when:**<br>• `Product.division` **bug fix**: `Product.division` and `DopplerDepartment.division` both return the true division shape (via `DivisionService`), not the department shape<br>• `Product.division` **bug fix**: The client-shape-change risk is logged with the PO and confirmed via a client survey before rollout<br>• Simple user/status fields + trivial pass-throughs: user fields resolve (null id → null)<br>• Simple user/status fields + trivial pass-throughs: `productThumbnailId` re-fetches<br>• Simple user/status fields + trivial pass-throughs: scalars mapped | — |
