...
|---|---|---|
| 🔴🔬 `SPARK-PROD-C01` — `getProducts(...)` two-stage hydration | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D01` — `addProduct` | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D02` — `addProducts` (bulk) | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D03` — `bulkUpdateProducts` | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D04` — `updateProduct` | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D06` — `addTeamsToProduct` 🔀 Collab Canvas | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D07` — `addBusinessPartnersToProductWithType` 🔀 Collab Canvas | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
| 🔴🔬 `SPARK-PROD-D11` — `updateWorkspaceAttributes` 🔀 Collab Canvas | `SPARK-SPIKE-06` | Cross-Domain Association / Hydration |
...
|---|---|---|---|---|---|
| 🔴🔬 🔷 `SPARK-PROD-C01`<br>`getProducts(...)` two-stage hydration<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟠 High `L` | Query<br>Calls: `search` | SPARK-SPIKE-06 | **Intent —** List products by combining the search index with the canonical record (two-stage hydration).<br>**Today —** listing products needs data from two places — the search index (which - knows flags like "has boms", "has claims", workspace membership) and the canonical product…<br>**Done when:**<br>• parity for 4 arg combos (no flags / all flags / resourceType=workspaces / filter array)<br>• truthy defaults preserved<br>• elastic flags merged onto canonical<br>• Workspace-filter placement and elastic/canonical staleness handling match `SPARK-SPIKE-06`'s decision | ☐ 4 combos<br>☐ default truthiness<br>☐ merge<br>☐ Parity: DGS response matches spark-internal-graphql baseline |
...
| Story | Complexity | Type | Depends On | Acceptance Criteria |
|---|---|---|---|---|
| 🔴🔬 🔶 `SPARK-PROD-D01`<br>`addProduct`<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟡 Medium `M` | Mutation<br>Calls: `workspaceV2`, `attachment` | SPARK-SPIKE-06 | **Intent —** Create a product (optionally copy from another + associate a workspace).<br>**Today —** POST ${v1} + optional copyProductToProduct(copyProduct) + workspace association<br>**Done when:**<br>• creates product<br>• optional copy runs when `copyProduct` present<br>• workspace assoc applied |
| 🔴🔬 🔶 `SPARK-PROD-D02`<br>`addProducts` (bulk)<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟡 Medium `M` | Mutation<br>Calls: `attachment` | SPARK-SPIKE-06 | **Intent —** Create many products at once (plus attachment links).<br>**Today —** bulk POST ${v1}/bulk + attachment-link side-effects (no rollback — preserve, flag)<br>**Done when:**<br>• bulk creates<br>• attachment links applied; no-rollback behaviour documented |
| 🔴🔬 🔶 `SPARK-PROD-D03`<br>`bulkUpdateProducts`<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟡 Medium `M` | Mutation | SPARK-SPIKE-06 | **Intent —** Update many products in one call.<br>**Today —** PUT ${v1}/mass_update<br>**Done when:**<br>• mass-updates products |
| 🔴🔬 🔶 `SPARK-PROD-D04`<br>`updateProduct`<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟡 Medium `M` | Mutation<br>Calls: `attachment` | SPARK-SPIKE-06 | **Intent —** Edit a product (optional copy + template-attachment cleanup).<br>**Today —** PUT ${v1}/{id} + optional copy + archive removed-template attachments (template branch)<br>**Done when:**<br>• updates product<br>• optional copy<br>• removed-template attachments archived (template branch) |
...
| 🔴🔬 🔶 `SPARK-PROD-D06`<br>`addTeamsToProduct` 🔀 Collab Canvas<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟢 Low `XS` | Mutation | SPARK-SPIKE-06 | **Intent —** Adds teams (and their partners) to a product.<br>**Today —** POST ${v1}/{productId}/resources/bulk + manage_workspace_teams<br>**Done when:**<br>• adds teams + new partners + workspace links |
| 🔴🔬 🔶 `SPARK-PROD-D07`<br>`addBusinessPartnersToProductWithType` 🔀 Collab Canvas<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟢 Low `XS` | Mutation | SPARK-SPIKE-06 | **Intent —** Adds business partners to a product with a given type.<br>**Today —** POST ${v1}/{productId}/partners-add/bulk<br>**Done when:**<br>• adds partners with type |
...
| 🔴🔬 🔶 `SPARK-PROD-D11`<br>`updateWorkspaceAttributes` 🔀 Collab Canvas<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06` (Cross-Domain Association / Hydration) — see global Spike Detail_ | 🟢 Low `XS` | Mutation | SPARK-SPIKE-06 | **Intent —** Updates a product's workspace attributes.<br>**Today —** PUT ${v1}/{productId} workspace attrs<br>**Done when:**<br>• updates workspace attrs |
