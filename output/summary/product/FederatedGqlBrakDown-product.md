...
| 🔴🔬 `SPARK-PROD-D03` — `bulkUpdateProducts` | `SPARK-SPIKE-06b` (via `SPARK-PROD-S01`) | Cross-Domain Association |
| 🔴🔬 `SPARK-PROD-D04` — `updateProduct` | `SPARK-SPIKE-06b` (via `SPARK-PROD-S01`) | Cross-Domain Association |
| 🔴🔬 `SPARK-PROD-D06` — `addTeamsToProduct` 🔀 Collab Canvas | `SPARK-SPIKE-06b` (via `SPARK-PROD-S01`) | Cross-Domain Association |
| 🔴🔬 `SPARK-PROD-D07` — `addBusinessPartnersToProductWithType` 🔀 Collab Canvas | `SPARK-SPIKE-06b` (via `SPARK-PROD-S01`) | Cross-Domain Association |
| 🔴🔬 `SPARK-PROD-D11` — `updateWorkspaceAttributes` 🔀 Collab Canvas | `SPARK-SPIKE-06b` (via `SPARK-PROD-S01`) | Cross-Domain Association |
...
| D | Mutations (simple) | 18 | 25–40d | after B01; D01-D04/D06/D07/D11 gated on `SPARK-SPIKE-06b` (Association) |
...
| 🔴🔬 🔶 `SPARK-PROD-D03`<br>`bulkUpdateProducts`<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06b` (Cross-Domain Association) — see global Spike Detail_ | 🟡 Medium `M` | Mutation | SPARK-SPIKE-06b | **Intent —** Update many products in one call.<br>**Today —** PUT ${v1}/mass_update<br>**Done when:**<br>• mass-updates products |
...
| 🔴🔬 🔶 `SPARK-PROD-D06`<br>`addTeamsToProduct` 🔀 Collab Canvas<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06b` (Cross-Domain Association) — see global Spike Detail_ | 🟢 Low `XS` | Mutation | SPARK-SPIKE-06b | **Intent —** Adds teams (and their partners) to a product.<br>**Today —** POST ${v1}/{productId}/resources/bulk + manage_workspace_teams<br>**Done when:**<br>• adds teams + new partners + workspace links |
| 🔴🔬 🔶 `SPARK-PROD-D07`<br>`addBusinessPartnersToProductWithType` 🔀 Collab Canvas<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06b` (Cross-Domain Association) — see global Spike Detail_ | 🟢 Low `XS` | Mutation | SPARK-SPIKE-06b | **Intent —** Adds business partners to a product with a given type.<br>**Today —** POST ${v1}/{productId}/partners-add/bulk<br>**Done when:**<br>• adds partners with type |
...
| 🔴🔬 🔶 `SPARK-PROD-D11`<br>`updateWorkspaceAttributes` 🔀 Collab Canvas<br>🔴🔬 _Spike-gated on `SPARK-SPIKE-06b` (Cross-Domain Association) — see global Spike Detail_ | 🟢 Low `XS` | Mutation | SPARK-SPIKE-06b | **Intent —** Updates a product's workspace attributes.<br>**Today —** PUT ${v1}/{productId} workspace attrs<br>**Done when:**<br>• updates workspace attrs |
