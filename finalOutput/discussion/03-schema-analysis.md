...
The target schema is translated from the two source SDLs (`SPARK_Discussion.txt` + `SPARK_DiscussionV3.txt`),
verified against the resolvers. **Discussion is its own subgraph** (`plm-discussion`) — every reference is
cross-subgraph.
...
11 queries + 26 mutations preserved (`0 ✅ | 37 🔜`); **3 schema-drift mutations deferred ⏭**
(`dropPartnerFromDiscussionIds`, `unDropPartnerFromDiscussionIds` — service methods run inside
`workspaceBusinessPartnerActionsV2`; `coreAddBulkDiscussionV3` — no resolver). `SPARK_`/`V2`/`V3` naming
preserved (client contract). **`core*` twins** are system-context versions of the public ops — keep both.
...
| Attachment coupling on write (bulk attachment input) | Low | Medium | Attachment client / entity refs | Architect |
