...
> Field types/nullability are taken from the source SDL (`code/schemas/SPARK_Attachment.txt`).
...
relationshipMetadata, previewable}` and the remaining scalar value-type fields — DTO-mapped. **Once A02
normalizes the dual record shape, most "Computed coalesce" fields above become plain Direct fields.**
