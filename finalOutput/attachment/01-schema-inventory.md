...
> **Source of truth:** `code/schemas/SPARK_Attachment.txt` (222-line SDL) + `code/resolvers/SPARK_Attachment.txt` (318) + `code/services/Attachment.txt` (403)
...
No `context.js`. `AttachmentService extends SparkService` takes the base URL **directly**
(`this.endpoint = endpoint`, `Attachment.txt:15`); paths are like `/attachments/v3?humanIds=`. The REST
service is **`plm-attachment`** (per platform direction).
...
| File | Lines | Role |
|---|---|---|
| `code/schemas/SPARK_Attachment.txt` | 222 | the source SDL — 9 queries, 15 mutations, `Attachment` + gallery/copy/access types, ~15 inputs |
| `code/resolvers/SPARK_Attachment.txt` | 318 | 8 queries (+1 drift), 15 mutations, `Attachment` (~25 field resolvers) + 4 sub-type blocks |
| `code/services/Attachment.txt` | 403 | ~30 REST methods (`attachments/v3` + renders/gallery/acl) |
| **Total** | **943** | mid domain — no chunked reading |

Schema: **`code/schemas/SPARK_Attachment.txt` (222 lines)** — target schema in [03-schema.graphql](./03-schema.graphql)
translated from it (nullability from the SDL).

## 3. Import Graph
```
SPARK_Attachment.txt
...
and search (which returns `SearchAttachment`). All outbound calls (above) are cross-subgraph.
...
getRendersForAttachmentIds(⚠@deprecated but resolved), getRendersForAttachmentV3Ids, getRendersForAttachmentIdsByPost,
getAttachmentsfromRelatedResource, getAttachmentsfromRelatedResources. **Drift ⏭:** getAttachments.
