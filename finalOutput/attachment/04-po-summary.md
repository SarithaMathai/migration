...
It is **mid-sized**: 8 queries (+1 deprecated drift), 15 mutations, ~32 field resolvers on a 318-line
resolver, with **no polymorphism**. The defining wrinkle is the **dual record shape** — the backend serves
attachments in both elastic `snake_case` and api `camelCase`, so ~18 field resolvers coalesce today; the DGS
port should normalize that at the DTO boundary.
...
> One engineer ≈ **10–17 sprints**. Parallelizable after A.
...
|---|---|---|
| 1–2 | A01–A04 + B01–B05 | schema (canonical DTO), service port, reads |
...
| 2 engineers | ~6–10 sprints | reads + mutations parallel after A |
