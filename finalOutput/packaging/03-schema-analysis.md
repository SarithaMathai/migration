...
The target schema is translated from the source SDL (`code/schemas/SPARK_Packaging.txt`), verified against
...
`@deprecated` fields/inputs (`retailPrice`, `creativePath`, `resourceType`, `fileDeliveryEmail`,
`Dieline.attachments`) are carried across with the directive. **Claims pass-through:** `PackagingInput`
carries `claimId`/`claimDetails` — confirm whether this stays here or moves with the claims subgraph.
...
| Claims pass-through fields on `PackagingInput` | Low | Low | Confirm ownership (packaging vs claims subgraph) | Architect |
