...
> **Source of truth:** `code/schemas/SPARK_Packaging.txt` (453-line SDL) + `code/resolvers/product/SPARK_Packaging.txt` + `code/services/product/Packaging.txt`
...
(`services/product/Packaging.txt:10`):
...
| File | Lines | Role |
|---|---|---|
| `code/schemas/SPARK_Packaging.txt` | 453 | the source SDL — 7 queries, 10 mutations, `Packaging`/`Dieline`/`PrinterDieline`/`PackagingElement` + ~20 value types, ~20 inputs |
| `code/resolvers/product/SPARK_Packaging.txt` | 273 | 7 queries, 10 mutations, 4 type blocks (17 field resolvers) |
| `code/services/product/Packaging.txt` | 164 | REST methods on `packaging/v1` + dielines + export |
| **Total** | **890** | mid domain — no chunked reading |

Schema: **`code/schemas/SPARK_Packaging.txt` (453 lines)** — target schema in [03-schema.graphql](./03-schema.graphql)
translated from it (nullability from the SDL). The SDL is **wide** (~24 object types, ~20 inputs).
...
PackagingService.txt uses postOne, putOne, loadListing, loadOne, convertFunctions
```
