...
> **Source of truth:** `code/schemas/SPARK_Search.txt` (SDL), `code/resolvers/SPARK_Search.txt` (507), `code/services/Search.txt` (1,034)
> **Depends on:** [01-schema-inventory.md](./01-schema-inventory.md) · **Mode:** Full (queries grouped by family)

Implementation spec. ACL/JWT usage is **context-only**. `search` is the elastic read hub (its own subgraph).
...
## Service Classes
`SearchService` base = `plm-elastic-search`. ~80 methods, each building an elastic query string/body and
posting/loading; transforms via `deepToCamelCase`. Counts/group-by helpers (`keyValuesTransform`).
