...
> Field types/nullability are taken from the source SDL (`code/schemas/SPARK_Impression.txt`).
...
|------|-----------|--------------|------------|--------------|-----------|------------|-------|
| `Impression` | `businessPartners` | `[VMM_BusinessPartner]` | EXT | `SPARK_Impression.txt:31` | 🔵 vmm | Low | G01 |
...
owningPartnerId, owningPartnerType, workspaceContext, deletable, associatedBomIds, attachmentId,
relatedResources, sortOrder, createdAt, updatedAt` — all DTO-mapped, no resolver. Covered by A02 schema +
B01/B02 mapping. (`partnerIds` exists on the data record and is read internally by `businessPartners`/
`counts`, but is **not** an exposed field in the SDL `SPARK_Impression` type.)
