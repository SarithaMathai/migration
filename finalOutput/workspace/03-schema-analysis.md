...
The target schema is translated from the source SDL (`code/schemas/SPARK_WorkspaceV2.txt`), verified against
the resolver. **Workspace is its own subgraph** (`plm-workspace`) — every reference is cross-subgraph.
...
8 queries + 10 mutations preserved (`0 ✅ | 20 🔜`); **2 schema-drift wrappers deferred ⏭**
(`dropWorkspaceBusinessPartnerV2`, `unDropWorkspaceBusinessPartnerV2` — traffic routes through
`workspaceBusinessPartnerActionsV2`). `SPARK_`/`V2` naming preserved exactly (client contract); `VMM_`/`IG_` kept.
...
| Cross-subgraph coupling to product (`products`, `addResources`) | Medium | Medium | Replace direct resolver calls with entity refs / a product client | Architect |
...
## 6. ACL Handling
The dispatcher curries capability tokens and calls ACL `dropPartnerFromResources`/`unDropPartnerFromResources`
with manual compensation. **ACL is ignored in the DGS implementation** (no ACL story) — context only; but the
