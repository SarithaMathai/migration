...
# Complexity -> story points (AI-derived; confirm in refinement). See jira/README.md.
POINTS = {"Low": 2, "Medium": 3, "High": 5, "Very High": 8}

CSV_HEADER = [
    "Issue Type", "Story ID", "Summary", "Epic Name", "Epic Link",
    "Phase", "Complexity", "Story point estimate",
...
def domain_rows(dm):
    ...
    for s in dm["stories"]:
        ...
        rows.append([
            ...
            s.get("phase", ""), s.get("complexity", ""),
            POINTS.get(s.get("complexity"), ""),
...
def write_stories_md(path: Path, dm):
    ...
    lines = [f"# {domain} — Jira stories (paste one block per issue)",
             ...
             "> Story points are AI-derived from complexity (confirm in refinement). See "
             "[README.md](./README.md).",
             ""]
    for s in dm["stories"]:
        sid = s["id"]
        ...
        lines += [
            ...
            (f"**Type:** Story  ·  **Phase:** {s.get('phase','')}  ·  "
             ...
             f"**Points (est.):** {POINTS.get(s.get('complexity'),'?')}  ·  "
             f"**Depends on:** {deps}"),
