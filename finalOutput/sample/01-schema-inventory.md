...
> **Source of truth:** `code/schemas/SPARK_SampleV2.txt` (513-line SDL) + `code/resolvers/SPARK_SampleV2.txt` (430) + `code/services/SampleV2.txt` (309)
...
| File | Lines | Role |
|---|---|---|
| `code/schemas/SPARK_SampleV2.txt` | 513 | the source SDL — 23 queries, 12 mutations, `SampleV2` + ~25 value types, ~10 inputs, 1 union |
| `code/resolvers/SPARK_SampleV2.txt` | 430 | 23 queries, 9 mutations, `SampleV2` (~35 field resolvers) + a `__resolveType` union + 6 sub-type blocks |
| `code/services/SampleV2.txt` | 309 | ~30 REST methods (`samples/v2` + export) |
| **Total** | **1,252** | mid-large domain — no chunked reading needed |

Schema: **`code/schemas/SPARK_SampleV2.txt` (513 lines)** — target schema in [03-schema.graphql](./03-schema.graphql)
translated from it (nullability from the SDL).

## 3. Import Graph
```
SPARK_SampleV2.txt imports:
...
createSampleRoundV2, bulkEvaluateSamples, requestSampleExport, retrySampleNotificationError,
retryAllSampleNotificationErrors, bulkCloneFilesForEvaluate. **Deferred ⏭:** updateSampleEvaluations,
dropSamples, undropSamples.
