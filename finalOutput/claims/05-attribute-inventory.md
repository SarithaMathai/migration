...
> Field types/nullability are taken from the source SDL (`code/schemas/SPARK_Claims.txt`).
...
manualLock, subjectiveStatements, workspaceContext, claimDetails, createdAt, updatedAt, partners}`;
`ClaimDetails.{id, claimCategory, technicalClaim, claimId, claimDescription, guestFacingClaim,
guestFacingClaimId, appliesTo, status, communicationChannel, shareWithGuests, claimsAbout, packagingCopy}`;
all of `ClaimPackagingCopy`, `ClaimSubstantiate.{substantiated, substantiatedAt}`, `ClaimExport`,
`Guest_Facing`, `CommunicationChannel` — DTO-mapped, no resolver. Covered by A02 + B01/B02.
...
subgraph every reference (Product, search, workspace, user-profile, VMM) is cross-subgraph. Cost
concentrates in `updateClaim` (proxy-ACL multi-step, E01) and the `parentDetails` elastic lookups (G03).
No polymorphism.
