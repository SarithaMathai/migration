...
> **Source of truth:** `code/schemas/SPARK_ProductDetail.txt` (139-line SDL) + `code/resolvers/product/SPARK_ProductDetail.txt` + `code/services/product/ProductDetails.txt`
...
constructor (`services/product/ProductDetails.txt:11`):
...
| File | Lines | Role |
|---|---|---|
| `code/schemas/SPARK_ProductDetail.txt` | 139 | the source SDL — 2 queries, 6 mutations, `ProductDetails`/item/category types, 8 inputs |
| `code/resolvers/product/SPARK_ProductDetail.txt` | 129 | 2 queries, 6 mutations, 3 type blocks (12 field resolvers) |
| `code/services/product/ProductDetails.txt` | 102 | 9 REST methods (`construction/v1`) — 2 unused (versions) |
| **Total** | **370** | small domain — no chunked reading |

Schema: **`code/schemas/SPARK_ProductDetail.txt` (139 lines)** — target schema in
[03-schema.graphql](./03-schema.graphql) is translated from it (nullability from the SDL).

## 3. Import Graph
```
SPARK_ProductDetail.txt
...
ProductDetailsService.txt uses postOne, putOne, loadListing, loadOne, convertFunctions
```
