...
7 queries + 8 mutations preserved with **exact operation names** (`0 ✅ | 15 🔜 | 0 ⏭`). `SPARK_` dropped;
the source type `SPARK_Measurements` → `Measurement`. `TightFit` uses a composite key `id version`.
