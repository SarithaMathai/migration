...
> Field types/nullability are taken from the source SDL (`code/schemas/SPARK_Measurement.txt`).
...
|------|-----------|--------------|------------|--------------|-----------|------------|-------|
| `Measurement` | `access` | `AccessControl` | EXT | `SPARK_Measurement.txt:95-101` | accessControl (context) | Low | G01 |
...
type, baseType, manualLock, version, status, statuses, toleranceSet, defaultUnitOfMeasure, coreSizes,
rows, workspaceContext, createdAt, updatedAt` — DTO-mapped, no resolver. Covered by A02 + B01 mapping.
(`statusId`/`statusName`/`businessPartnerIds` exist on the data record and are read internally to build
`status` and `businessPartners`, but are **not** exposed fields in the SDL `SPARK_Measurements` type.)
