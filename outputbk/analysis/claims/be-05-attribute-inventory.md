...
| `Claims` | `status` | `CodeDescription` | Direct | — | Low | B01 |
| `Claims` | `statuses` | `[ProductComponentStatus]` | Direct | — | Low | B01 |
...
- Covered by B01/B02 (no dedicated resolver story — `A02` in earlier drafts of this table referenced a
  foundation-phase story that claims' be-04-stories.md never authored; these are plain DTO fields on the
  root query's own response).
