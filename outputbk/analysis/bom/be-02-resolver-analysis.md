...
without opening the `.js` resolver. Every cross-domain call is tagged 🔴/🟡/🔵.
...
## Helper Functions (module-level, `resolvers/SPARK_Bom.js:710-735` + `:63-80`)

### H1 · `getMaterialWeight(material, ctx)` — `:710-719`
- Used by every material type's `weight` field.
- If `material.weight` set: load `materialHub.getHubUnitsOfMeasure`, find UoM by `weightUomId` (fallback code **23** = grams), return `{value, unitOfMeasure}`.
- Else `null`.
...
- `matId = material.libraryResource.id`; if present → `permissionJWT = await getUserPermissionsJWT(matId)` → `materialHub.getHubMaterial(permissionJWT).load(matId)`.
- **EXT:** 🔴 `accessControl` + 🟡 `materialHub`.
...
- TRIM(4)→`BomTrimMaterial` · WASH(6)→`BomWashMaterial` · FABRIC(2)→`BomFabricMaterial` · COMBINATION(15)→`BomCombinationMaterial` · FABRIC_SPEC(16)→`BomFabricSpecMaterial` · PACKAGING+others(10,11,12,13,14,17–24)→`BomPackagingMaterial` · default(1 COMPONENT, 5 OTHER, **9 HUB**)→`BomMaterial`.
- **Preserve the HUB-falls-through default.** DGS: `@DgsTypeResolver`.
...
### S1 · `BomService` — base `…/enterprise_product_development_products/bom/v1` — `services/product/Bom.js` (184 lines)
...
**Latent bugs to fix on port:** `getHubMaterial` missing `await` on `getUserPermissionsJWT` (`utils/bomUtils.js:271`);
