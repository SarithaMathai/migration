...
| **T-Shirt Size** | **M** |
| **Total Stories** | 20 |
| **Complexity** | 🔴 0 Very High · 🟠 1 High · 🟡 5 Medium · 🟢 14 Low |
...
| 2 | 🟢 `B-02`, 🟢 `B-03`, 🟢 `B-04`, 🟢 `B-05`, 🟡 `C-01`, 🟢 `C-02`, 🟡 `D-01`, 🟢 `D-02`, 🟢 `D-03`, 🟢 `D-04`, 🟢 `D-05`, 🟡 `D-06`, 🟢 `D-07`, 🟠 `E-01`, 🟡 `F-01`, 🟡 `G-01`, 🟢 `G-02`, 🟢 `H-01` | `E-01` → 🔬 SPIKE-01 · ⛔ BLOCKED-BY product (PRODUCT-BE-E-00, the shared WriteSaga module)<br>`H-01` → ⛔ BLOCKED-BY sample | Fan-out — 📖 Core Reads · 🔍 Search & Listing · ✏️ Mutations · ⚙️ Complex Operations · 🔗 Federation & Stitching · 🧪 Field Resolvers & Tests · 🧬 Entity Resolution |
| 3 | 🟢 `G-04` | — | 🧪 Field Resolvers & Tests |

**Critical path:** `B-01` → `B-02` → `G-04` — 3 sequential stories; everything else hangs off this chain in parallel.
...
**BE-1:** `B-01` → `E-01` → `B-02` → `B-05` → `C-01` → `D-01` → `D-06` → `F-01` → `G-01` → `B-03` → `B-04` → `C-02` → `D-02` → `D-03` → `D-04` → `D-05` → `D-07` → `G-02` → `G-04` → `H-01`

**Elapsed (nominal midpoints):** ~42 working days.
...
##### 🧬 Phase H — Entity Resolution (1 stories)
