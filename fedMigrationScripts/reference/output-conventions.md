...
| 6 | `be-06-cross-domain-field-analysis.md` | `skills/cross-domain-field-analysis/SKILL.md` |
| 7 | `be-07-acl-usage-analysis.md` | `skills/acl-usage-analysis/SKILL.md` |

**Domain directory naming:** kebab-case matching the loader key (e.g., `product`, `bom`, `product-details`).

**Program roll-ups (not per-domain):** `output/schemaAnalysis/00-cross-domain-field-inventory.md`
(aggregates every domain's be-06) and `output/aclResearch/00-acl-usage-inventory.md` (aggregates
every domain's be-07) — generated by `generate_schema_analysis.py` / `generate_acl_analysis.py`
respectively, run after Phase 2 (and Phase 3 for be-06, since it links to `be-03-schema.graphql`).
...
## 14. Recommendation Vocabularies (Phases 6 and 7)

Use exactly these phrases — never paraphrase — so findings stay greppable across all 8 domains.

### Phase 6 — federation recommendation (`be-06-cross-domain-field-analysis.md`)

| Recommendation | When |
|---|---|
| `@requires field composition` | Single co-located phase-1 domain (same `plm-product` subgraph) |
| `Co-locate + @requires` | Multiple phase-1 domains feed one field; sequence the `@requires` chain |
| `Entity resolver (@key + @DgsEntityFetcher)` | Dependency lives in a separate DGS subgraph |
| `Gateway stitch (@extends stub)` | External platform (VMM/IG/Doppler/Corona/etc); gateway resolves, no DGS migration |
| `No cross-domain dependency` | Resolves locally |
| `Needs manual review — unclassified loader key` | The loader key is missing from `domain-service-catalog.md` — fix the catalog, don't guess |

### Phase 7 — ACL classification + recommendation (`be-07-acl-usage-analysis.md`)

| Classification | Recommendation |
|---|---|
| `permission-check` | `No action needed — permission check only, resolves locally, no token handoff` |
| `own-domain-token` | `No action needed — token gates the resolver's own read/write, stays resolver-local` |
| `downstream-token` (target = phase-1/sibling DGS domain) | `Mid-Request ACL Update — SparkSecurityService.updateCurrentUserPermissions(capabilityToken) refreshes the thread's security context before the downstream call, avoiding re-authentication` |
| `downstream-token` (target = external platform) | `Gateway-level auth passthrough — downstream is an external platform stub; the gateway carries the original auth header, no mid-request refresh needed` |
| `unresolved-token` | `Needs manual review — token minted but downstream use not statically resolved` |

**Every `downstream-token` finding is a documented conflict** with the "ACL is context-only,
ignored" language in be-03/be-04 — Phase 7 output flags this explicitly but never edits
be-03/be-04 itself; that doc rewrite is a separate, later, reviewed pass.
