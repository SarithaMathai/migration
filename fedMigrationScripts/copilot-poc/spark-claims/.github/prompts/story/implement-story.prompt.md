...
description: "Implement one SPARK-CLM-* migration story end-to-end (schema + Kotlin fetcher + service + tests)"
---

Implement the migration story **${input:storyId:SPARK-CLM-B01}**.

Follow the `story-implementer` workflow (`.github/chatmodes/story-implementer.chatmode.md`):

1. Gate check — if the story is `SPARK-CLM-E01` (or any future Phase-E story) or depends on a `SPARK-SPIKE-0x`, stop and report the open spike instead of coding (use `/check-spike-gate` logic). If the story is `SPARK-CLM-F01` or `SPARK-CLM-F02`, confirm the owning type (`Product` / `ResourcesCount`) already exists in `plm-product` before proceeding — they are BLOCKED-BY that.
