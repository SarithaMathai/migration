...
Write the parity tests for **${input:operation:getClaims}** (story ${input:storyId:SPARK-CLM-B01}).
...
2. Pull the story's *Test Cases* checklist — for High / Very High stories (e.g. `SPARK-CLM-E01`) every checklist item becomes one named test; Medium / Low get happy path + one backend-error path.
3. Write Kotlin tests per `.github/instructions/kotlin/testing.instructions.md`:
   ...
   - federation contributions (F01/F02): test entity resolution given a representation for the owning type's key
