...
- This subgraph also **contributes fields into `plm-product`'s types**:
  - `extend type Product @key(fields: "id") { claims(...): [Claims] }` (`CLAIM-BE-F-01`)
  - `extend type ResourcesCount @key(fields: "productId partnerId") { claims: [ID] }` (`CLAIM-BE-F-02`, the TechPack rollup)
...
- Match the target schema in `output/analysis/claims/be-03-schema.graphql` at https://github.com/XXX; if the implementation must diverge, call it out in the PR description.
