...
- Work items are Jira stories with ids like `SPARK-PROD-B02`, `SPARK-BOM-D03`, `SPARK-WL-G01` (prefixes: `PROD`, `BOM`, `MEAS`, `PKG`, `IMP`, `PDTL`, `WL`).
...
- `B01` of each domain lands the one-time module scaffold (`{domain}.graphqls`, scalar registration, service + client wiring). All later B/C/D/G stories assume it and are mutually parallelizable.
...
- **Spike gating:** a story flagged 🔴🔬 or listing `SPARK-SPIKE-0x` in *Depends On* must not have its complex part implemented until that spike's decision is recorded. Every Phase-E story is gated on `SPARK-SPIKE-01` (non-atomic write saga) unless stated otherwise. When in doubt run the `check-spike-gate` prompt.
...
- **No invented rollback.** Multi-step writes keep today's behaviour until `SPARK-SPIKE-01` concludes; do not add compensation logic on your own.
...
- Commit messages: `SPARK-PROD-B02: <what changed>` — story id first.
