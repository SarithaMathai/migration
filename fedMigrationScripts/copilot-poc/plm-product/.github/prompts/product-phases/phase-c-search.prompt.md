...
description: "Phase C — a search/listing query story (e.g. SPARK-PROD-C01) in apps/app"
---

Implement Phase-C story **${input:storyId:SPARK-PROD-C01}** in `apps/app`.
...
1. **Gate check first** — if *Depends on* names a `SPARK-SPIKE-0x` or an `S0x` spike (e.g. `SPARK-PROD-C01` depends on `S02`, the `getProducts` hydration spike), run `/check-spike-gate` before implementing the hydration/merge logic. If the spike is still open, the story's own text usually names a **safe default** ("until S02 concludes, preserve today's shape") — implement that default, not a guess at the eventual design.
...
Most Phase-C stories in `product` are gated on `SPARK-SPIKE-06a` (hydration) via `SPARK-PROD-S02` — always check.
