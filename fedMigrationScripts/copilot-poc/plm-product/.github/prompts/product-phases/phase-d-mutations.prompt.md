...
description: "Phase D — a simple mutation story (e.g. PRODUCT-BE-D-01) in apps/app"
---

Implement Phase-D story **${input:storyId:PRODUCT-BE-D-01}** in `apps/app`.
...
1. **Gate check first** — run `/check-spike-gate ${input:storyId}`. Most `product` D-stories (`D-01`–`D-04`, `D-06`, `D-07`, `D-11`) depend on `S-01`/`SPIKE-06b` (cross-domain association pattern). If open, the story names a fallback ("orchestrate create→copy→assoc per S-01's chosen pattern" — implement today's ad-hoc sequence as the interim, don't invent a saga).
