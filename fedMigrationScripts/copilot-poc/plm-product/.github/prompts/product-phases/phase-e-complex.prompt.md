...
description: "Phase E — a complex multi-step write story (e.g. SPARK-PROD-E01) in apps/app"
---

Implement Phase-E story **${input:storyId:SPARK-PROD-E01}** in `apps/app`.

Phase E is **Complex Operations** — multi-step orchestrated writes, aggregations, or facades. **Every Phase-E story is spike-gated by default** (`SPARK-SPIKE-01`, non-atomic write saga, unless overridden to `02`/`03`/`04` — see the override table).
...
5. Implement per `.github/instructions/kotlin/datafetcher.instructions.md` / `service.instructions.md`. If the story is large (Very High complexity), expect it to be split into sub-tasks in Jira (`E01-1`, `E01-2`, …) — implement and PR one sub-task at a time, not the whole story in one diff.
