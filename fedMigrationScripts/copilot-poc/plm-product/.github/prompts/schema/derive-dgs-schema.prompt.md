...
1. Locate the target definition in `output/analysis/{domain}/be-03-schema.graphql` at https://github.com/XXX — that file is the derived target for this subgraph; copy its intent, don't re-derive from the legacy SDL.
...
5. Report: the schema diff, any divergence from `be-03-schema.graphql` (with reason), and which story's data fetcher must ship with it (schema never merges alone).
